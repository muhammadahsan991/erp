...
class ErpController extends Controller
{
    public function actionIndex()
    {
        ...
        if (count($agingReport) > 0) {
            ...
            if (file_exists($erpReportFileName)) {
                $erpModel->createAgingReportCsvFileHeader($file, [
                    ...
                    '1-8' => $eightDays . '-' . $currentDate,
                    '9-15' => $sixDays . '-' . $startDateSixDaysReport,
                    '16-30' => $thirtyDays . '-' . $startDateThirtyDaysReport,
                    '31-90' => $sixtyDays . '-' . $startDateSixtyDaysReport,
                    '91-180' => $ninetyDays . '-' . $startDateNinetyDaysReport,
                    '181-365' => $hundredEightyDays . '-' . $startDateOneEightyDaysReport,
        ...
        if (count($agingReportInternal) > 0) {
            ...
            if (file_exists($agingReportInternalFileName)) {
                $erpModel->createAgingReportInternalCsvFileHeader($agingReportFileInternal, [
                    'Delivery Company' => 'Company',
                    '1-60' => $sixtyDaysInternal.'-'.$currentDate,
                    '61-180' => $oneTwentyDaysInternal.'-'.$startDateSixtyDaysReportInternal,
                    '181-365' => $previousYear.'-'.$startDateOneEightyDaysReportInternal,
                    'more365' => '> 365 Days',
                    'Outstanding' => 'Outstanding Amount'
                ]);
        ...
        if (!$erpModel->logCronCompleted("Report Generated Successfully")) {
    ...
    public function actionInsert()
    {
        ...
        if ($erpModel->insertErpReceiptsData('ErpReceipts.csv')) {
            if (!$erpModel->logCronCompleted("Erp Receipts Data Inserted Successfully")) {
                $erpModel->createLogFile($erpModel->ERROR_FILE, 'Log Erp Receipts Data insertion Failed'."\n");
            }
        }

        if ($erpModel->insertErpOmsData('ErpOms.csv')) {
            if (!$erpModel->logCronCompleted("Erp OMS Data Inserted Successfully")) {
